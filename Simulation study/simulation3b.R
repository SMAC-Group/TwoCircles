...
for (k in 1:2){
  ...
  for (i in 1:length(delta)){
    for (j in 1:B){
      ...
      # Simulation sample
      x = runif(n1[k], 0, 2*pi)
      y1 = rvm(n2[k], 0, 0.0000001+delta[i])
      y2 = rvm(n2[k], 2*pi/3, 0.0000001+delta[i])
      y3 = rvm(n2[k], 4*pi/3, 0.0000001+delta[i])

      w = purrr::rdunif(n2[k], a = 1, b = 3)
      y = (w==1)*y1 + (w==2)*y2 + (w==3)*y2
...
save(out, file = "Simulation study/simulation3b.RData")
