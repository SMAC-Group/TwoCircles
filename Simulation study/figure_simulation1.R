load("Simulation study/simulation1.RData")
...
tikz("Simulation study/tex/simu1.tex", width = 7, height = 3.5, standAlone = TRUE,
...
plot(NA, ylim = c(0,1), xlim = range(mu), main = "dg", xlab = "x", ylab = "y" , cex.axis = 0.7, cex = 0.7, cex.lab = 0.8, ann = FALSE, axes = FALSE)
grid(col = "grey85")
mtext("Rejection frequency of H$_0$", side = 2, line = 2, cex=taille.leg, col = "grey30")
mtext("$\\mu$", side = 1, line = 2, cex=taille.leg, col = "grey30")
box(col = "black")
title("\\textbf{(a)}")
...
k = 1
lines(mu, out$res_rao[k,]/100, col = coleur[2], lty = 1)
lines(mu, out$res_dixon[k,]/100, col = coleur[3], lty = 2)
lines(mu, out$res_WW[k,]/100, col = coleur[4], lty = 4)
...
legend(1.25,0.34,c("Rao","Dixon","Wheeler-Watson"),
